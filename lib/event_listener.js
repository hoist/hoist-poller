...
EventListener.prototype.createListeners = function (endpoint) {
  ...
  if (this.bucket) {
    options.bucketId = this.bucket._id;
  }
  emitter.on(newEvent, function (payload) {
    logger.info(modifiedEvent, 'poller-event-listener newEvent-before send');
    options.eventName = newEvent;
    options.payload = payload;
    var ev = new EventBroker.events.ApplicationEvent(options);
    logger.info(options, 'inside poller-event-listener newEvent-before send');
    return eventBroker.send(ev).then(function () {
      return ev;
    });
  });
  emitter.on(modifiedEvent, function (payload) {
    logger.info(modifiedEvent, 'poller-event-listener modifiedEvent');
    options.eventName = modifiedEvent;
    options.payload = payload;
    var ev = new EventBroker.events.ApplicationEvent(options);
    logger.info(options, 'inside poller-event-listener modifiedEvent-before send');
    return eventBroker.send(ev).then(function () {
      return ev;
    }).catch(function (err) {
      console.log('err', err);
