...
Poller.prototype = {
  start: function () {
    ...
    return Model.Subscription.findAsync({})
      ...
      .then(function (subscriptions) {
        ...
        logger.info('AFTER SETTLE', promises);
  ...
  hydrateSubscription: function (subscription) {
    logger.info('HYDRATE SUBSCRIPTION', subscription._id);
    var self = this;
      ...
      .then(function (connector) {
        ...
        return Model.Application.findOneAsync({
          ...
          .then(function (app) {
            ...
            if (connector.settings.authType === 'Private') {
              logger.info(connector.settings, 'inside hoist-poller#hydrateSubscription authType === Private');
              return self.pollModule(app, subscription, connector);
            } //else {
            // var query = {
            //   application: app._id,
            //   environment: subscription.environment,
            ...
            //           return self.pollModule(app, subscription, connector, bouncer, bucket);
            //         }));
            //       });
            //   });
            // }
  ...
  pollModule: function (app, subscription, connector, bouncer, bucket) {
    var resolve, reject;
    var promise = new BBPromise(function () {
      resolve = arguments[0];
      reject = arguments[1];
    });
    ...
    _subscription.eventEmitter.on('done', function () {
      logger.info('poll listener received done');
    ...
    // send bouncer and bucket when not private app
    require(connectorPath)(app.toObject(), _subscription, connector.toObject());
    return promise;
